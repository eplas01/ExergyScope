...
from exergyscope import ampl_syntax, print_set, print_df, newline, print_param, print_header, print_run
...
def print_data(config):
    ...
    if config['printing']:
        ...
        exergy_cost = config['exergy']
        NR_cost = config['NR_cost']
        carbon_cost = config['carbon_cost']
        # Pre-processing df #
        ...
        ELEC_CATEGORIES = ["MEC_DRIVE", "OTHERS_ELEC", "LIGHT", "HEAT_RES"]
        TECHNO_MOBIL = ['BUS_COACH_DIESEL', 'BUS_COACH_HYDIESEL', 'BUS_COACH_CNG_STOICH', 'BUS_COACH_FC_HYBRIDH2',
                        'CAR_GASOLINE', 'CAR_DIESEL',
                        'CAR_NG', 'CAR_METHANOL', 'CAR_FUEL_CELL', 'BOAT_FREIGHT_DIESEL', 'BOAT_FREIGHT_NG',
                        'BOAT_FREIGHT_METHANOL', 'TRUCK_DIESEL', 'TRUCK_METHANOL', 'TRUCK_FUEL_CELL',
                        'TRUCK_NG', 'TRAMWAY_TROLLEY', 'TRAIN_PUB', 'CAR_HEV', 'CAR_PHEV',
                      'CAR_BEV', 'TRAIN_FREIGHT', 'TRUCK_ELEC'];  # List of technologies for mobility
        eud_simple.loc['ELECTRICITY'] = np.sum(-eud_simple.loc[i] * layers_in_out.loc[i, "ELECTRICITY"] for i in ELEC_CATEGORIES)
        eud_sankey = eud_simple.astype('float')
        eud_simple = eud_simple.drop(ELEC_CATEGORIES)
        eud_simple = eud_simple.astype('float')
        layers_in_out_elec = layers_in_out.loc[ELEC_CATEGORIES + TECHNO_MOBIL].astype('float')
        layers_in_out_elec = pd.DataFrame(layers_in_out_elec)
        #technologies.drop(ELEC_CATEGORIES + ['TRANSPORT'], inplace=True)
        layers_in_out.drop(ELEC_CATEGORIES, inplace=True)
        layers_in_out.drop(ELEC_CATEGORIES, axis=1, inplace=True)
        layers_in_out = layers_in_out.astype('float')
        ...
        #SECTORS_SANKEY = list(eud_sankey.columns)
        END_USES_SANKEY = list(eud_sankey.index)
        ...
        for i in END_USES_TYPES:
            li = list(layers_in_out_tech.loc[layers_in_out_tech.loc[:, i] == 1, :].index) #TO MODIFY
        ...
        INFRASTRUCTURE = [item for item in ALL_TECHS if item not in STORAGE_TECH and item not in ALL_TECH_OF_EUT  and item not in ELEC_CATEGORIES]
        ...
        eud_sankey = ampl_syntax(eud_sankey, '')
        share_ned = ampl_syntax(share_ned, '')
        #content = ampl_syntax(content, '')
        layers_in_out = ampl_syntax(layers_in_out, '')
        layers_in_out_elec = ampl_syntax(layers_in_out_elec, '')
        ...
        # printing sets
        #print_param('exergy_demand')





        print_set(ELEC_CATEGORIES, 'ELEC_CATEGORIES', out_path)
        print_set(TECHNO_MOBIL, 'TECHNO_MOBIL', out_path)
        #print_param('exergy_destruction_final2useful', sum(exergy_destruction), 'Exergy destruction final to useful', out_path)
        print_set(SECTORS, 'SECTORS', out_path)
        print_set(END_USES_INPUT, 'END_USES_INPUT', out_path)
        print_set(END_USES_SANKEY, 'END_USES_SANKEY', out_path)
        ...
        print_param('exergy_cost', exergy_cost, 'exergy destruction cost', out_path)
        print_param('NR_cost', NR_cost, 'non-renewable cost', out_path)
        print_param('carbon_cost', carbon_cost, 'carbon cost', out_path)
        ...
        newline(out_path)
        print_df('param end_uses_demand_year_sankey : ', eud_sankey, out_path)
        newline(out_path)
        ...
        newline(out_path)
        #print_df('param:', content, out_path)
        #newline(out_path)
        with open(out_path, mode='a', newline='') as file:
            writer = csv.writer(file, delimiter='\t', quotechar=' ', quoting=csv.QUOTE_MINIMAL)
        ...
        with open(out_path, mode='a', newline='') as file:
            writer = csv.writer(file, delimiter='\t', quotechar=' ', quoting=csv.QUOTE_MINIMAL)
            writer.writerow(['# Link between layers  (data from Tables 19,21,22,23,25,29,30)'])
        print_df('param layers_in_out_elec : ', layers_in_out_elec, out_path)
        newline(out_path)
        with open(out_path, mode='a', newline='') as file:
            writer = csv.writer(file, delimiter='\t', quotechar=' ', quoting=csv.QUOTE_MINIMAL)
            writer.writerow(
