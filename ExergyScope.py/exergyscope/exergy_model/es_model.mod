# -------------------------------------------------------------------------------------------------------------------------													
#	ExergyScope TD is an open-source energy model suitable for country scale analysis. It is a simplified representation of an urban or national energy system accounting for the energy flows
...
set TECHNO_MOBIL; # technologies related to mobility
#################################
...
param end_uses_demand_year_sankey {END_USES_SANKEY, SECTORS} >= 0 default 0; # end_uses_year [GWh]: table end-uses demand vs sectors (input to the model). Yearly values. [Mpkm] or [Mtkm] for passenger or freight mobility.
...
param end_uses_sankey {i in END_USES_SANKEY} := sum {s in SECTORS} (end_uses_demand_year_sankey [i,s]);
...
param exergy_cost >=0; # [Meuros/GWh] cost of exergy destruction
param NR_cost >=0; # [Meuros/GWh] cost of non-renewable resources
param carbon_cost>=0; # [Meuros/ktCO2-eq.] cost of carbon
...
param renewable{TECHNOLOGIES} >= 0, <= 1 default 0;
param final2useful{TECHNOLOGIES} >= 0, <= 1 default 0;
...
param layers_in_out {RESOURCES union TECHNOLOGIES diff STORAGE_TECH diff ELEC_CATEGORIES, LAYERS}; # f: input/output Resources/Technologies to Layers. Reference is one unit ([GW] or [Mpkm/h] or [Mtkm/h]) of (main) output of the resource/technology. input to layer (output of technology) > 0.
param layers_in_out_elec {ELEC_CATEGORIES union TECHNO_MOBIL, LAYERS union ELEC_CATEGORIES}; # f: input/output Resources/Technologies to Layers. Reference is one unit ([GW] or [Mpkm/h] or [Mtkm/h]) of (main) output of the resource/technology. input to layer (output of technology) > 0.
...
var Exergy_Destruction >= 0;
var Exergy_Tax_NonRenewable >=0;
var Exergy_Input >=0;
var Exergy_Output >=0;
var Final2usefulDes >=0;
...
subject to end_uses_t {l in LAYERS, h in HOURS, td in TYPICAL_DAYS}:
	End_uses [l, h, td] = (if l == "ELECTRICITY" 
		then
			(end_uses_input["ELECTRICITY"] * electricity_time_series [h, td] / t_op [h, td] ) + Network_losses [l,h,td]
		else (if l == "FOOD_COOLING" then
			end_uses_input["FOOD_COOLING"] / total_time
		else (if l == "SPACE_COOLING" then # TO MODIFY : Space cooling variable
			(end_uses_input["SPACE_COOLING"] * cooling_time_series [h, td] / t_op [h, td])
		else (if l == "PROCESS_COOLING" then
			end_uses_input["PROCESS_COOLING"] / total_time
...
subject to totalcost_cal:
	TotalCost = sum {j in TECHNOLOGIES} (tau [j]  * C_inv [j] + C_maint [j]) + sum {i in RESOURCES} C_op [i];
		#+ Exergy_Tax_NonRenewable*0.001 Exergy_Destruction;
...
subject to layer_balance {l in LAYERS, h in HOURS, td in TYPICAL_DAYS}:
		sum {i in RESOURCES union TECHNOLOGIES diff STORAGE_TECH diff ELEC_CATEGORIES}
...
## Exergy Analysis Added for the thesis
subject to exergyInput:
	Exergy_Input = sum{i in RESOURCES diff{'ELEC_EXPORT', 'CO2_EMISSIONS'}, t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]}
		(F_t [i, h, td] * t_op [h, td]);
subject to exergyOutput:
	Exergy_Output= sum {i in END_USES_SANKEY diff {'ELECTRICITY', 'MOBILITY_PASSENGER', 'MOBILITY_FREIGHT', 'NON_ENERGY'}}
		(end_uses_sankey[i])
		+ sum {i in TECHNO_MOBIL,
         l in LAYERS diff {'CO2_ATM', 'CO2_INDUSTRY', 'CO2_CAPTURED', 'MOB_PUBLIC', 'MOB_PRIVATE','MOB_FREIGHT_RAIL','MOB_FREIGHT_ROAD','MOB_FREIGHT_BOAT'},
         t in PERIODS,
         h in HOUR_OF_PERIOD[t],
         td in TYPICAL_DAY_OF_PERIOD[t]} (-layers_in_out[i, l] * F_t[i, h, td] * layers_in_out_elec[i, 'MEC_DRIVE'])
		+ sum{t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]}(
		 -layers_in_out["ELEC_EXPORT","ELECTRICITY"] * F_t ["ELEC_EXPORT", h, td] )
		+ sum{i in END_USES_TYPES_OF_CATEGORY["NON_ENERGY"]}(
			end_uses_input["NON_ENERGY"] * share_ned [i] )
		;

## Exergy
subject to exergyDestruction:
	Exergy_Destruction = Exergy_Input- Exergy_Output;
subject to Final2usefulDestruction:
	Final2usefulDes = sum {i in TECHNOLOGIES diff STORAGE_TECH diff TECHNO_MOBIL diff ELEC_CATEGORIES}(
		sum {l in LAYERS diff{'CO2_ATM', 'CO2_INDUSTRY', 'CO2_CAPTURED','MOB_PUBLIC', 'MOB_PRIVATE','MOB_FREIGHT_RAIL','MOB_FREIGHT_ROAD','MOB_FREIGHT_BOAT'}, t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]}(- layers_in_out[i, l] * F_t[i, h, td])*final2useful[i]
	)
		+ sum {i in TECHNO_MOBIL,
         l in LAYERS diff {'CO2_ATM', 'CO2_INDUSTRY', 'CO2_CAPTURED', 'MOB_PUBLIC', 'MOB_PRIVATE','MOB_FREIGHT_RAIL','MOB_FREIGHT_ROAD','MOB_FREIGHT_BOAT'},
         t in PERIODS,
         h in HOUR_OF_PERIOD[t],
         td in TYPICAL_DAY_OF_PERIOD[t]} (-layers_in_out[i, l] * F_t[i, h, td] * (1-layers_in_out_elec[i, 'MEC_DRIVE']))
		;

subject to exergyDestructionTax:
	Exergy_Tax_NonRenewable = sum{i in RESOURCES diff RE_RESOURCES, t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]}(
		F_t[i, h, td] * t_op [h, td]);
...
minimize obj:TotalCost + Exergy_Tax_NonRenewable * NR_cost + Exergy_Destruction * exergy_cost + TotalGWP*carbon_cost;
