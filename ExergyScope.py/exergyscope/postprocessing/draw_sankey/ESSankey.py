...
def drawSankey(path="./",outputfile='TO_REPLACE',auto_open=True):
    ...
    if outputfile == 'TO_REPLACE':
        outputfile = path / 'python_generated_sankey_test.html'
    ...
    fig = genSankey(flows,cat_cols=['source','target'],value_cols='realValue',title='Exergy',color_col='layerColor')
    fig.write_html(str(outputfile), auto_open=auto_open)
    #fig.to_image(format="png", engine="kaleido")

    fig.write_image(str(path / 'python_generated_sankey_test.pdf'), format="pdf", height=700, width=1400)
    fig.show()
...
def genSankey(df,cat_cols=[],value_cols='',title='Sankey Diagram',color_col=[]):
    ...
    sourceTargetDf2 = sourceTargetDf.groupby('target', as_index=False)['count'].sum()
    sourceTargetDf3 = sourceTargetDf.groupby('source', as_index=False)['count'].sum()
    for i in labelList:
        if i not in list(sourceTargetDf2['target']):
            sourceTargetDf2.loc[len(sourceTargetDf2)] = [i, sourceTargetDf3.loc[sourceTargetDf3['source']==i]['count'].values[0] ]#sourceTargetDf3[count].loc[sourceTargetDf3['source'] == i].values[0]]
    labelList2 = list("%s (%.2f)" % (sourceTargetDf2['target'][i], sourceTargetDf2['count'][i]) for i in range(len(sourceTargetDf2['target'])))
    targetList = list(sourceTargetDf2['target'])

    # add index for source-target pair
    sourceTargetDf['sourceID'] = sourceTargetDf['source'].apply(lambda x: targetList.index(x))
    sourceTargetDf['targetID'] = sourceTargetDf['target'].apply(lambda x: targetList.index(x))
